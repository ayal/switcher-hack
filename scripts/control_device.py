...
async def control_thermostat(
    ...
    print("Controlling thermo", device_id, device_key, device_ip, remote_id, 'state', possible_states[state], 'mode',  possible_modes[mode], 'temp', target_temp, 'fan', possible_fan_level[fan_level], 'swing', swing, 'update', update_state, SwitcherBreezeRemoteManager().get_remote(remote_id))
    async with SwitcherType2Api(device_ip, device_id, device_key) as api:
        printer.pprint(
            asdict(
