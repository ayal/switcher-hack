...
# set logger level to not log anything
...
class SwitcherApi(ABC):
    ...
    async def connect(self) -> None:
        ...
        print("connecting to", self._ip_address, self._port, "...")
...
class SwitcherType2Api(SwitcherApi):
    ...
    async def control_breeze_device(
        ...
        """
        # print("control_breeze_device", state, mode, target_temp, fan_level, swing, update_state)
        ...
        # print("all login response", hexlify(login_resp.unparsed_response))
        ...
        if (
            ...
            if update_state:
                ...
                # print("sending a set status packet", packet)
            else:
                ...
                """ print(
                    ...
                    "\n\nfan_level.value:", fan_level.value,
                    "\n\nset_swing.value:", set_swing.value,
                ) """
                ...
                # print("\n\nsending a control packet to breeze, before sign and len\n\n\n", packet, "\n\n\n", "length:", len(packet))

            # logger.debug("setting the message length")
            # packet = set_message_length(packet)
            logger.debug("signing the packet")

            signed_packet = sign_packet_with_crc_key(packet)

            # print("sending a control packet after length and sign\n\n", signed_packet)
            self._writer.write(unhexlify(signed_packet))
            logger.debug("reading the response")
            response = await self._reader.read(1024)
            logger.debug("parsing the response")
            cmd_response = SwitcherBaseResponse(response)
            logger.debug("response: %s", cmd_response)
